...
public class ArbolExpresionGrafico extends JPanel 
{
     ...
     * @param n:Objeto de la clase Nodo <T> que se utiliza como
    ...
    private void calcularPosicion(Nodo n, int left, int right, int top) 
    {
      ...
      StudentsModel stud = (StudentsModel)  n.getContent();
      int width = fm.stringWidth(stud.getIdStudents()+"");
    ...
    private void dibujarArbol(Graphics2D g, Nodo n, int puntox, int puntoy, int yoffs) 
    {
     ...
     g.draw(r);
     StudentsModel stud = (StudentsModel)  n.getContent();
     g.drawString(stud.getIdStudents()+"", r.x + 3, r.y + yoffs);
